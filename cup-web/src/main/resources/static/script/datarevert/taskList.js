$(function() {
	...
	jQuery(grid_selector).jqGrid({
		...
		height : gridHeight,
		colNames : [ 'id', '系统编码', '代理程序', '任务名称', '表名', '执行条件', '目标分组', '终止时间', '状态'],
		colModel : [ {
			name : 'id',
			width : 50,
			hidden : true
	...
	jQuery(grid_selector_detail).jqGrid({
		datatype : "json",
		height : gridHeight,
		colNames : [ 'id', '系统编码', '代理程序', '表名', '分组', '启动时间', '结束时间', '已恢复量'],
		colModel : [ {
			name : 'id',
			width : 50,
			hidden : true
	...
	$("#addTaskBtn").on('click', function() {
		$('#taskModal').load("job/gotoDataRevertEdit").modal();
	});
	...
	$("#updateTaskBtn").on('click', function() {
		...
		if (id == '') {
			...
			$('#taskModal').load("job/gotoDataRevertEdit?id=" + id).modal();
		}
	});
