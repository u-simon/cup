$(function() {
	...
	jQuery(grid_selector).jqGrid({
		url : "associateJob/selectByConditions",
		datatype : "json",
		mtype : 'POST',
		postData : {
			"name" : $("#query_name").val(),
	...
	var queryJobDetail = function() {
		...
		jQuery("#grid-table_detail").jqGrid('setGridParam', {
			url : "associateJob/selectDetailByMids",
			postData : {
				"sysCode" : rowData["sysCode"],
				"agentCode" : rowData["agentCode"],
				"tbName" : rowData["tbName"],
	...
	$("#addTaskBtn").on('click', function() {
		$('#taskModal').load("associateJob/gotoTaskEdit").modal();
	});

    /** 导入按钮* */
    $("#importTaskBtn").on('click', function() {
        $('#importModel').load("associateJob/gotoImportEdit").modal();
    });
	...
	$("#updateTaskBtn").on('click', function() {
		...
		if (id == '') {
			...
			$('#taskModal').load("associateJob/gotoTaskEdit?id=" + id).modal();
		}
	});
		...
		if (id == '') {
			...
			swal({
				...
				jQuery.ajax({
					type : "post",
					dataType : "json",
					url : "associateJob/modifyStatusById",
					data : param,
					success : function(data) {
						if (data.success == true) {
							swal({
